...
beforeAll(async () => {
  ...
  userMemberAuthenticated = await createMember({
    ...memberCreateBody,
    clientId: userClientAuthenticated.id,
    roleId: roleIdMember,
  });
...
describe("SWAP BETWEEN CLIENT AND MEMBER", () => {
  const basePathRouter = "/api/v1/resources/users/clients/changeAddress";
  describe("SWAP AS ADMIN", () => {
    test(
      `When an Admin access POST ${basePathRouter}` +
        " sending in body the clientId and the memberId that will be swapped," +
